...
class TinyDB(appContext: Context?) {
    ...
    fun getListObject(key: String?): ArrayList<FoodDomain2> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList = ArrayList<FoodDomain2>()
        for (jObjString in objStrings) {
            val player: FoodDomain2 = gson.fromJson(jObjString, FoodDomain2::class.java)
    ...
    fun putListObject(key: String?, playerList: ArrayList<FoodDomain2>) {
        checkForNullKey(key)
        val gson = Gson()
